...
class DeepSeekApi(object):
    ...
    def retry_translate_title(self, tr_title, max_retries=10, delay=2):
        ...
        while retries < max_retries:
            try:
                print(f"deepseek translate title the {retries}th retry")
                # 调用API
                completion = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "你是一个中英文翻译专家，将用户输入的英文翻译成中文，保留英文人名和地名。翻译的字数应当和英文字数相匹配，字数上不要相差太多，用户可以向助手发送需要翻译的内容，助手会回答相应的翻译结果，并确保符合中文语言习惯，调整语气和风格为新闻类型的，并考虑到某些词语的文化内涵和地区差异。同时作为翻译家，需将原文翻译成具有信达雅标准的译文。\"信\" 即忠实于原文的内容与意图；\"达\" 意味着译文应通顺易懂，表达清晰；\"雅\" 则追求译文的文化审美和语言的优美。目标是创作出既忠于原作精神，又符合目标语言文化和读者审美的翻译。"
                        },
                        {
                            "role": "user",
                            # "content": f"将标题翻译成中文, 不要返回翻译说明, 将要翻译的标题为: {tr_title}",
                            "content": tr_title
                        }
                    ],
                    max_tokens=512,
    ...
    def retry_translate_content(self, tr_content, max_retries=10, delay=2):
        ...
        while retries < max_retries:
            try:
                print(f"deepseek translate content and the {retries}th retry")
                ...
                completion = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "你是一个中英文翻译专家，用户会利用爬虫技术爬取很多新闻类的网页内容，用户想将内容翻译成中文，翻译的时候保留html标签，保留英文人名和地名。并且在内容的最后总结全篇的思想，总结性的内容用单独一个div标签包裹，另外不用出来总结两个字，总结性的中文内容不超过200字。翻译的内容要确保符合中文语言习惯，利用新闻风格来调整语气和风格，并考虑到某些词语的文化内涵和地区差异。同时作为翻译家，需将原文翻译成具有信达雅标准的译文。\"信\" 即忠实于原文的内容与意图；\"达\" 意味着译文应通顺易懂，表达清晰；\"雅\" 则追求译文的文化审美和语言的优美。目标是创作出既忠于原作精神，又符合目标语言文化和读者审美的翻译。"
                        },
                        {
                            "role": "user",
                            "content": tr_content,
                ...
                end_time = time.time()
                total_time = end_time - start_time
                print(f"deepseek translate content took total time：{total_time:.2f} s")
            ...
            retries += 1
            print(f"deepseek translate title failed and we will sleep and try again: {retries}th.")
            if retries < max_retries:
                time.sleep(delay)
            else:
                print(f"deepseek finally we translated too many times {max_retries} and give up this translator.")
                return None
